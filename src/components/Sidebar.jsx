...
const ICONS = {
  ...
  "edit-square": <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>,
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 px-4 py-5 border-b border-white/10">
        ...
        {!collapsed && (
          <span className="text-white font-semibold text-sm leading-tight">Grupo<br/>Cofisem</span>
        )}
      </div>
      ...
      <nav className="flex-1 overflow-y-auto overflow-x-hidden py-3">
        {navItems.map((item) => {
          // Comparación exacta para evitar que /siniestros active /siniestros/nuevo y viceversa
          ...
          return (
            <NavLink
              ...
              className={`flex items-center gap-3 mx-2 my-0.5 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-150 group cursor-pointer
                ${isActive ? "bg-white text-[#13193a]" : "text-white/70 hover:bg-white/10 hover:text-white"}`}
            >
        ...
        <div className="flex items-center gap-3 px-2 py-2">
          ...
          {!collapsed && (
            <div className="flex-1 min-w-0">
              ...
              <p className="text-white/40 text-[10px] truncate">{rolNombre?.replace(/_/g, " ")}</p>
            </div>
          )}
        </div>
        <button
