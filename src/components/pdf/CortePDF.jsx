...
const tdCheck = { fontFamily: "Helvetica-Bold", fontSize: 6.5, color: "#0f7a4a" };
...
function FilaPoliza({ r, i }) {
  ...
  return (
    <Fila style={{ backgroundColor: bg, borderBottomWidth: 0.5, borderBottomColor: COLORS.rule }} wrap={false}>
      {COLS_POLIZA.map((c) => (
        <Celda key={c.key} col={c} textStyle={c.key === "poliza" || c.key === "pago" ? tdBold : td}>
          {val[c.key]}
        </Celda>
      ))}
    </Fila>
...
function FilaPago({ r, i }) {
  ...
  const val = {
    ...
    fotos: r.fotos_url || r.fotos_verificado ? "✓" : "—",
    ...
    <Fila style={{ backgroundColor: bg, borderBottomWidth: 0.5, borderBottomColor: COLORS.rule }} wrap={false}>
      {COLS_PAGO.map((c) => (
        <Celda
          ...
          col={c}
          textStyle={
            c.key === "efectivo" ? tdBold
              : marcados.includes(c.key) ? (val[c.key] === "✓" ? tdCheck : tdDash)
              : td
          }
        >
          {val[c.key]}
...
function FilaTotalesPoliza({ totales }) {
  const v = {
    no: "", aseguradora: "", poliza: "", fEmision: "", vigInicio: "", vigFin: "", folio: "",
    vendedor: "", asegurado: "TOTAL",
    vale: $(totales.vale),
    primaAnual: $(totales.primaAnual),
    primaNeta: $(totales.primaNeta),
    cuota: "",
    pago: $(totales.primerPago),
    cobertura: "", placas: "", tipo: "",
  };
  return (
...
function FilaTotalesPago({ totales }) {
  const v = {
    ...
    cheque: "", tdc: "", autorizacion: "", polPendPago: "", telefono: "", observaciones: "",
...
function TablaSeccion({ titulo, children }) {
  return (
    <View style={{ marginBottom: 10 }}>
      <Text style={{ fontFamily: "Helvetica-Bold", fontSize: 8, color: COLORS.navy, marginBottom: 3 }}>
        {titulo}
      </Text>
      <View style={{ borderWidth: 0.5, borderColor: COLORS.rule }}>{children}</View>
    </View>
  );
}
  ...
  return (
    <Document>
      <Page size="A4" orientation="landscape" style={{ paddingHorizontal: 20, paddingVertical: 18, fontFamily: "Helvetica", backgroundColor: COLORS.white }}>
        ...
        <Fila style={{ borderTopWidth: 1, borderTopColor: COLORS.rule, borderBottomWidth: 1, borderBottomColor: COLORS.rule, paddingVertical: 5, marginBottom: 10, justifyContent: "space-between" }}>
        ...
        <TablaSeccion titulo="PAGO Y RESPALDO">
          ...
          {registros.length === 0 ? (
            ...
            <>
              {registros.map((r, i) => <FilaPago key={r.id ?? i} r={r} i={i} />)}
