...
export async function fetchClientes() {
  const { data, error } = await supabase
    .from("clientes")
    .select("id, nombre, apellido, rfc, curp, telefono, email, direccion, calle, numero_ext, numero_int, colonia, ciudad, estado, cp, activo, polizas(id)")
    .order("nombre");
...
export async function crearCliente(form, creadoPor) {
  const { data, error } = await supabase
    ...
    .insert({
      ...
      numero_ext: form.numeroExt?.trim() || null,
      numero_int: form.numeroInt?.trim() || null,
      direccion:  [form.calle?.trim(), form.numeroExt?.trim(), form.numeroInt?.trim() ? `Int. ${form.numeroInt.trim()}` : null].filter(Boolean).join(" ") || null,
