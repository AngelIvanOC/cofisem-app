  ...
  async function registrarEnCorte(finalPoliza) {
    try {
      ...
      const { error } = await supabase.from('polizas_cofisem').upsert({
        ...
        asegurado_apellido_paterno: (cliente.apellido || '').split(' ')[0] || null,
        asegurado_apellido_materno: (cliente.apellido || '').split(' ').slice(1).join(' ') || null,
        telefono:          cliente.telefono || null,
        vendedor_id:       vendedor.id ?? 1,
        vendedor_nombre,
        placas:            finalPoliza.placas || null,
        num_serie:         finalPoliza.num_serie || null,
