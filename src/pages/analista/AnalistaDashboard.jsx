...
// Solo KPIs clave + atajos a secciones
// ============================================================
...
// ── Estas tarjetas son clickeables y llevan a la sección ──────
const KPIS = [
  { label: "Pólizas vigentes",   value: "186",  accent: "emerald", path: "/polizas",      sub: "+5 esta semana"    },
  { label: "Pend. de aplicar",   value: "4",    accent: "blue",    path: "/polizas",      sub: "Requieren acción"  },
  { label: "Cuotas vencidas",    value: "7",    accent: "red",     path: "/pagos",        sub: "Sin cobrar"        },
  { label: "Por vencer (7d)",    value: "5",    accent: "amber",   path: "/polizas",      sub: "Próximas a expirar"},
  { label: "Cobrado este mes",   value: "$186k",accent: "emerald", path: "/reportes",     sub: "Todas las oficinas"},
  { label: "Cortes pendientes",  value: "1/4",  accent: "amber",   path: "/corte-diario", sub: "1 oficina abierta" },
];
...
const ACCENT_MAP = {
  emerald: { card: "bg-emerald-50 border-emerald-200 text-emerald-700" },
  blue:    { card: "bg-blue-50    border-blue-200    text-blue-700"    },
  amber:   { card: "bg-amber-50   border-amber-200   text-amber-700"   },
  red:     { card: "bg-red-50     border-red-200     text-red-600"     },
};
...
export default function AnalistaDashboard({ usuario }) {
  ...
  return (
    // h-full + overflow-hidden = sin scroll
    <div className="h-full flex flex-col gap-6 p-6 bg-gray-50 overflow-hidden">
      ...
      {/* ── KPIs — 6 tarjetas clickeables ──────────────────── */}
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        {KPIS.map((k) => {
          const { card } = ACCENT_MAP[k.accent];
          return (
            <button
              key={k.label}
              onClick={() => navigate(k.path)}
              className={`${card} border rounded-2xl p-4 text-left hover:shadow-md hover:scale-[1.02] active:scale-[0.98] transition-all duration-150`}
            >
