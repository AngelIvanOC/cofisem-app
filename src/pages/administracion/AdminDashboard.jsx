...
// Dashboard limpio — ocupa la pantalla, sin scroll
// KPIs globales + alertas urgentes + atajos a secciones
// ============================================================
...
const KPIS = [
  { label: "Pólizas activas",     value: "186",   accent: "emerald", path: "/polizas",      sub: "+12 esta semana"   },
  { label: "Pend. autorizar",     value: "3",     accent: "blue",    path: "/pagos",        sub: "Pagos en espera"   },
  { label: "Endosos pendientes",  value: "2",     accent: "amber",   path: "/endosos",      sub: "Sin procesar"      },
  { label: "Cancelaciones hoy",   value: "1",     accent: "red",     path: "/polizas",      sub: "Revisar"           },
  { label: "Cobrado este mes",    value: "$186k", accent: "emerald", path: "/reportes",     sub: "Todas las oficinas"},
  { label: "Cortes del día",      value: "3/4",   accent: "amber",   path: "/corte-diario", sub: "1 pendiente"       },
  { label: "Usuarios activos",    value: "12",    accent: "blue",    path: "/usuarios",     sub: "4 oficinas"        },
  { label: "Por vencer (7d)",     value: "5",     accent: "red",     path: "/polizas",      sub: "Renovar pronto"    },
];

// Máximo 3 alertas — lo realmente urgente
const ALERTAS = [
  {
...
export default function AdminDashboard({ usuario }) {
  ...
  return (
    <div className="h-full flex flex-col gap-5 p-6 bg-gray-50 overflow-hidden">
      ...
      {/* ── KPIs — 8 tarjetas en 2 filas ───────────────────── */}
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-2.5">
        {KPIS.map((k) => (
          <button
            key={k.label}
            onClick={() => navigate(k.path)}
            className={`${ACCENT_MAP[k.accent]} border rounded-2xl p-3.5 text-left hover:shadow-md hover:scale-[1.02] active:scale-[0.98] transition-all duration-150`}
          >
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-100 shadow-sm flex flex-col overflow-hidden">
          ...
          <div className="flex-1 flex flex-col divide-y divide-gray-50 justify-evenly">
            {ALERTAS.map((al, i) => (
              ...
              >
                <div className={`w-2 h-2 rounded-full shrink-0 ${DOT_MAP[al.accent]}`} />
