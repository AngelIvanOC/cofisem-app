...
const COBERTURAS_CATALOGO = {
  "TAXI BÁSICA 2500": {
    ...
    primaNeta: 1496.55, derechos: 400.00, iva: 303.45, total: 2200.00,
  ...
  "TAXI BÁSICA PAGOS 2700": {
    ...
    primaNeta: 1600.00, derechos: 400.00, iva: 320.00, total: 2320.00,
  ...
  "SERV. PÚB. 50/50 GAMAN 2": {
    ...
    primaNeta: 1790.00, derechos: 400.00, iva: 358.00, total: 2548.00,
  ...
  "COBERTURA APP (UBER, DIDI)": {
    ...
    primaNeta: 2260.00, derechos: 450.00, iva: 432.80, total: 3142.80,
...
const VENDEDORES_MOCK = ["ADMINISTRADOR", "Laura Rosher", "Marco A. Cruz", "Carlos Soto"];

const POLIZAS_MOCK = [
  { id: "3413241", asegurado: "Angel Ivan Ortega",  cobertura: "COBERTURA APP (UBER, DIDI)", vendedor: "Laura Rosher",  prima: 3142.80, vence: "13/03/2027", estatus: "Vigente",  forma: "Trimestral",  uso: "DIDI" },
  { id: "3413198", asegurado: "María García López", cobertura: "TAXI BÁSICA 2500",           vendedor: "Marco A. Cruz", prima: 2200.00, vence: "12/03/2027", estatus: "Vigente",  forma: "Contado",     uso: "TAXI" },
  { id: "3413167", asegurado: "Roberto Díaz Ramos", cobertura: "SERV. PÚB. 50/50 GAMAN 2",  vendedor: "Laura Rosher",  prima: 2548.00, vence: "11/03/2027", estatus: "Vigente",  forma: "4 Parciales", uso: "TAXI" },
  { id: "3411002", asegurado: "Carmen López",       cobertura: "TAXI BÁSICA 2500",           vendedor: "Carlos Soto",   prima: 2200.00, vence: "20/03/2026", estatus: "Por vencer",forma: "Contado",    uso: "TAXI" },
];
  ...
  return (
    <button
      ...
      className={[
        ...
        active ? "border-[#13193a] text-[#13193a]" : "border-transparent text-gray-400 hover:text-gray-600",
      ].join(" ")}
    >
      {children}
    </button>
...
function FormCotizacion({ cotizacionInicial, onGuardar, onTramitar, onCancelar }) {
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <select value={form.vendedor} onChange={e => setF("vendedor", e.target.value)} className={inpCls}>
              {VENDEDORES_MOCK.map(v => <option key={v}>{v}</option>)}
            </select>
          </div>
          <div>
          ...
          <div className="sm:col-span-2">
            <label className={lblCls}>Cliente <span className="text-red-400">*</span></label>
            <select value={form.cliente} onChange={e => setF("cliente", e.target.value)} className={inpCls}>
              <option value="">Seleccione un cliente</option>
              {CLIENTES_MOCK.map(c => <option key={c}>{c}</option>)}
            </select>
          </div>
          <div className="sm:col-span-2">
            <label className={lblCls}>Cobertura <span className="text-red-400">*</span></label>
            <select value={form.cobertura} onChange={e => setF("cobertura", e.target.value)} className={inpCls}>
              <option value="">Seleccione una cobertura</option>
              {TIPOS_COBERTURA.map(c => <option key={c}>{c}</option>)}
            </select>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="bg-[#13193a] px-5 py-3 flex items-center justify-between">
            <div>
              ...
              <p className="text-white/40 text-xs mt-0.5">Uso: {coverturaData.uso}</p>
            </div>
          </div>
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="bg-gray-50/80 border-b border-gray-100">
                  <th className="text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wide px-4 py-2.5">Descripción</th>
                  <th className="text-right text-[11px] font-semibold text-gray-400 uppercase tracking-wide px-4 py-2.5">Monto Asegurado</th>
                  <th className="text-right text-[11px] font-semibold text-gray-400 uppercase tracking-wide px-4 py-2.5">% Ded.</th>
                  <th className="text-right text-[11px] font-semibold text-gray-400 uppercase tracking-wide px-4 py-2.5">Prima Neta</th>
                </tr>
              </thead>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div>
            ...
            <input value={form.codAMIS} onChange={e => setF("codAMIS", e.target.value)} placeholder="Código AMIS" className={inpCls}/>
          </div>
          <div className="sm:col-span-2">
            <label className={lblCls}>Buscar unidad por nombre</label>
            <input value={form.unidad} onChange={e => setF("unidad", e.target.value)} placeholder="Marca, modelo o descripción del vehículo" className={inpCls}/>
          </div>
          <div>
            <label className={lblCls}>Capacidad</label>
            <input value={form.capacidad} onChange={e => setF("capacidad", e.target.value)} placeholder="Pasajeros" className={inpCls}/>
          </div>
          <div>
            <label className={lblCls}>Modelo</label>
            <input value={form.modelo} onChange={e => setF("modelo", e.target.value)} placeholder="Año" className={inpCls}/>
          </div>
          <div>
            <label className={lblCls}>Inciso</label>
            <input value={form.inciso} onChange={e => setF("inciso", e.target.value)} placeholder="Inciso" className={inpCls}/>
          </div>
        </div>
      ...
      <div className="flex flex-wrap gap-3 pt-2">
        ...
        <button
          onClick={() => onGuardar({ id: nroCot, ...form, total, fecha: `${fechaHoy} ${horaHoy}`, vendedor: form.vendedor, guardada: true })}
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <button onClick={onCerrar} className="text-sm text-gray-400 hover:text-gray-600">← Volver</button>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            ...
            <tbody className="divide-y divide-gray-50">
              ...
              {["Resp. Civil a Terceros Bienes y Personas", "Resp. Civil Complementaria Personas", "Gastos Médicos Conductor y Familiares", "Muerte de Conductor x/AA", "Gastos Legales", "Resp. Civil Viajero"].map(desc => (
...
function TramiteExitoso({ cotizacion, onNueva, onVolver }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 px-8 text-center">
      ...
      <p className="text-gray-400 text-sm mb-8">Cliente: <strong className="text-gray-600">{cotizacion.cliente}</strong></p>
  ...
  if (tab === "exito" && tramiteOk) {
    return (
      <div className="p-6 min-h-full bg-gray-50">
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <TramiteExitoso cotizacion={tramiteOk} onNueva={() => { setTramiteOk(null); setCotActiva(null); setTab("nueva"); }} onVolver={() => { setTramiteOk(null); setTab("polizas"); }}/>
        </div>
      </div>
    );
  }
    ...
    <div className="p-6 min-h-full bg-gray-50">
      ...
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={() => { setCotActiva(null); setTab("nueva"); }}
      ...
      {tab === "comparar" && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          <Comparador
            ...
            onSeleccionar={tipo => { setCotActiva({ cobertura: tipo }); setTab("nueva"); }}
          />
        </div>
      )}
            ...
            <>
              <div className="flex flex-wrap items-center gap-2 px-5 py-4 border-b border-gray-100">
                <div className="relative">
                  ...
                  <input value={busqueda} onChange={e => setBusqueda(e.target.value)} placeholder="Buscar póliza o asegurado..."
                    ...
                    <tbody className="divide-y divide-gray-50">
                      {cotizaciones.map((c, i) => (
                        <tr key={i} className="hover:bg-gray-50/60 transition-colors">
                          ...
                          <td className="px-5 py-3.5 text-xs font-bold text-emerald-700">${c.total.toFixed(2)}</td>
