...
import { subirAudioSiniestro, getAudioSignedUrl } from "../../../services/evidencias";
...
export default function BotonAudioSiniestro({ siniestroId, numeroSiniestro, audioUrl, onUploaded, variant = "default" }) {
  ...
  const handleFile = async (e) => {
    ...
    setError(null);
    try {
      const path = await subirAudioSiniestro({ siniestroId, numeroSiniestro, file });
      onUploaded?.(path);
  ...
  return (
    <>
      <button
        ...
        className={btnCls}
      >
        {audioUrl ? <Volume2 className="w-3.5 h-3.5" /> : <Mic className="w-3.5 h-3.5" />}
        {variant === "resumen" && (audioUrl ? "Reproducir audio" : "Adjuntar audio a siniestro")}
      </button>
        ...
        >
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-bold text-[#13193a]">Audio del siniestro</h3>
              <button onClick={cerrar} className="w-7 h-7 rounded-lg hover:bg-gray-100 flex items-center justify-center text-gray-400">
                <X className="w-4 h-4" />
              </button>
            </div>
              ...
              <label className="flex flex-col items-center justify-center gap-2 py-8 border-2 border-dashed border-gray-200 rounded-xl cursor-pointer hover:border-[#13193a]/30 transition-all">
                {subiendo ? (
                  ...
                  <>
                    ...
                    <span className="text-xs text-gray-500 font-medium">Seleccionar o grabar audio</span>
                  </>
                )}
                <input
                  type="file"
