...
const SINIESTROS_INIT = [
  { folio: "SN-10234", asegurado: "Carlos Gómez",    vehiculo: "Toyota Corolla", fecha: "17/03/26", ubicacion: "Jiutepec, Mor",    ajustador: "Félix Hernández", estatus: "Activo"     },
  { folio: "SN-10231", asegurado: "Ana Martínez",    vehiculo: "Honda Civic",    fecha: "17/03/26", ubicacion: "Cuernavaca, Mor",  ajustador: null,              estatus: "Pendiente"  },
  { folio: "SN-10227", asegurado: "Roberto Díaz",    vehiculo: "Nissan Tsuru",   fecha: "17/03/26", ubicacion: "Jiutepec, Mor",    ajustador: "Félix Hernández", estatus: "Completado" },
  { folio: "SN-10220", asegurado: "Laura González",  vehiculo: "KIA Sportage",   fecha: "17/03/26", ubicacion: "Temixco, Mor",     ajustador: null,              estatus: "Pendiente"  },
  { folio: "SN-10215", asegurado: "Miguel Ortega",   vehiculo: "VW Jetta",       fecha: "16/03/26", ubicacion: "Jiutepec, Mor",    ajustador: "Luis Martínez",   estatus: "Completado" },
  { folio: "SN-10212", asegurado: "Pedro Ruiz",      vehiculo: "Chevrolet Aveo", fecha: "16/03/26", ubicacion: "Jiutepec, Mor",    ajustador: "Ana García",      estatus: "Activo"     },
  { folio: "SN-10208", asegurado: "Sofía Torres",    vehiculo: "Nissan Versa",   fecha: "15/03/26", ubicacion: "Jiutepec, Mor",    ajustador: "Luis Martínez",   estatus: "Activo"     },
];
    ...
    <div className="space-y-2 pt-2 border-t border-gray-100">
      ...
      <div className="space-y-1.5">
        {AJUSTADORES.map((aj) => {
          ...
          return (
            <button key={aj.id} onClick={() => !lleno && setSel(aj.id)} disabled={lleno}
              className={[
                "w-full flex items-center gap-2.5 p-2.5 rounded-xl border-2 text-left transition-all",
                sel === aj.id ? "border-[#13193a] bg-[#13193a]/5"
                  : lleno    ? "border-gray-100 bg-gray-50 opacity-50 cursor-not-allowed"
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex gap-0.5 mt-0.5">
                  {Array.from({ length: MAX }).map((_, i) => (
                    <div key={i} className={`w-2 h-2 rounded-full ${i < aj.activos ? "bg-blue-400" : "bg-gray-200"}`}/>
                  ))}
                </div>
              </div>
              {sel === aj.id && (
              ...
              {lleno && <span className="text-[9px] text-red-500 font-bold shrink-0">Lleno</span>}
            </button>
          );
        })}
      </div>
      <div className="flex gap-2 pt-1">
        <button onClick={onCancelar} className="flex-1 py-2 rounded-xl border border-gray-200 text-xs font-semibold text-gray-500 hover:bg-gray-50">
          Cancelar
        </button>
        <button onClick={confirmar} disabled={!sel || busy}
...
function ModalDetalle({ s, onClose, onAsignar }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden w-full max-w-5xl"
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                {[["Asegurado", s.asegurado], ["Vehículo", s.vehiculo], ["Fecha", s.fecha],
                  ["Teléfono", "777 100 2233"], ["Póliza", "3413241"], ["Cobertura", "TAXI BÁSICA 2500"]
                ].map(([l, v]) => (
                  <div key={l}>
            ...
            <div>
              ...
              <div className="grid grid-cols-4 gap-3">
                {[["Póliza","📄"],["Evidencia","📷"],["No. Serie","🪪"],["Licencia(s)","🪪"]].map(([l, ic]) => (
                ...
                <div key={i} className="flex gap-3">
                  <div className="flex flex-col items-center">
                    <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 ${e.done ? "border-emerald-500 bg-emerald-500" : "border-gray-300 bg-white"}`}>
                      {e.done && <svg className="w-2.5 h-2.5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="3.5"><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7"/></svg>}
                    </div>
                    {i < etapas.length - 1 && <div className={`w-0.5 h-7 my-1 rounded-full ${e.done ? "bg-emerald-200" : "bg-gray-200"}`}/>}
                  </div>
                  <div className="pb-1">
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-4 space-y-3">
              ...
              {s.ajustador && !modoAsignar ? (
                ...
                <div className="space-y-2">
                  ...
                  <button onClick={() => setModoAsignar(true)}
...
export default function Siniestros() {
  ...
  const filtrados = siniestros.filter((s) => {
    const mb = busqueda === "" || s.folio.toLowerCase().includes(busqueda.toLowerCase()) || s.asegurado.toLowerCase().includes(busqueda.toLowerCase());
    const me = filtroEstatus === "Todos" || s.estatus === filtroEstatus;
    return mb && me;
  });

  const asignar = (folio, nombreAjustador) => {
    setSiniestros((prev) => prev.map((s) => s.folio === folio ? { ...s, ajustador: nombreAjustador, estatus: "Asignado" } : s));
    setModalSiniestro((prev) => prev?.folio === folio ? { ...prev, ajustador: nombreAjustador, estatus: "Asignado" } : prev);
  };
    ...
    <div className="p-6 min-h-full bg-gray-50 space-y-5">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">Consulta, seguimiento y asignación de ajustadores</p>
        </div>
        {sinAsignar > 0 && (
          <div className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-red-50 border border-red-200">
            <span className="w-2 h-2 rounded-full bg-red-500 animate-pulse"/>
            <p className="text-xs font-bold text-red-700">{sinAsignar} {sinAsignar === 1 ? "siniestro" : "siniestros"} sin ajustador</p>
          </div>
        )}
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="flex flex-wrap items-center gap-2 px-5 py-4 border-b border-gray-100">
          <select value={filtroEstatus} onChange={(e) => setFiltroEstatus(e.target.value)}
            ...
            <option>Todos</option><option>Completado</option><option>Pendiente</option>
            <option>Cancelado</option><option>Activo</option><option>Asignado</option>
          </select>
          <div className="relative ml-auto">
            <svg className="w-3.5 h-3.5 text-gray-400 absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
              <path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z"/>
            </svg>
            <input value={busqueda} onChange={(e) => setBusqueda(e.target.value)} placeholder="Buscar folio o asegurado..."
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-50">
              {filtrados.length === 0 ? (
                <tr><td colSpan={8} className="text-center py-12 text-sm text-gray-400">No se encontraron siniestros.</td></tr>
              ) : filtrados.map((s, i) => (
                <tr key={i} onClick={() => setModalSiniestro(s)}
                  ...
                  <td className="px-5 py-3.5 text-gray-700 text-xs font-medium whitespace-nowrap">{s.asegurado}</td>
                  ...
                  <td className="px-5 py-3.5 text-xs">
                    {s.ajustador ? (
                      <span className="text-gray-700 font-medium">{s.ajustador}</span>
                    ) : (
                  ...
                  <td className="px-5 py-3.5">
                    <span className={`inline-flex text-[11px] font-semibold px-2.5 py-1 rounded-full ${STATUS_CLS[s.estatus] ?? "bg-gray-100 text-gray-600"}`}>
                      {s.estatus}
                    </span>
                  </td>
                  <td className="px-5 py-3.5" onClick={(e) => e.stopPropagation()}>
                    <div className="flex gap-1">
                      <button onClick={() => setModalSiniestro(s)} title="Ver detalle"
                      ...
                      {!s.ajustador && (
                        <button onClick={() => setModalSiniestro(s)} title="Asignar ajustador"
        ...
        <div className="flex items-center justify-between px-5 py-3 border-t border-gray-100">
          <p className="text-xs text-gray-400">Mostrando <strong>{filtrados.length}</strong> de <strong>{siniestros.length}</strong></p>
          <div className="flex gap-1">
            <button className="w-7 h-7 rounded-lg text-xs text-gray-400 hover:bg-gray-100">‹</button>
      ...
      {modalSiniestro && (
        <ModalDetalle s={modalSiniestro} onClose={() => setModalSiniestro(null)} onAsignar={asignar}/>
      )}
    </div>
  );
}
