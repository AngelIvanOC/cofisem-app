    ...
    <div className="p-3 sm:p-6 min-h-full bg-gray-50">
      ...
      <div className="flex items-start gap-3 sm:gap-4 mb-4 sm:mb-6">
        ...
        <div className="flex-1 min-w-0">
          <h1 className="text-xl sm:text-2xl font-bold text-[#13193a]">Reportar Siniestro</h1>
      ...
      {paso === "buscar" && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 sm:p-6">
          ...
          {/*
            En desktop: fila [input | botón].
            En móvil:   columna — input arriba, luego fila [badge-tipo | botón].
            sm:contents en el wrapper de badge+botón lo hace "transparente" en desktop,
            dejando el botón como hermano directo del input dentro del flex-row.
          */}
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
            {/* Input */}
            <div className="relative flex-1 min-w-0">
              {tipoDetect && (
                <div className="hidden sm:flex absolute left-3 top-1/2 -translate-y-1/2 items-center gap-1.5 pointer-events-none z-10">
              ...
              <input
                ...
                placeholder="Ej: A757LTS · 3N1EB31S · 01250100001024-01"
                className={[
                  "w-full h-11 rounded-xl border-2 text-sm text-gray-700 placeholder-gray-300 bg-white",
                  "focus:outline-none focus:ring-0 transition-all px-4",
                  tipoDetect ? "sm:pl-32" : "",
                  noEncontrado
            ...
            {/* En móvil: fila badge (izq) + botón (der). En desktop: sm:contents lo colapsa */}
            <div className={`flex items-center gap-2 sm:contents ${!tipoDetect ? "justify-end" : ""}`}>
              {tipoDetect && (
                <div className="sm:hidden flex items-center gap-1.5 flex-1 min-w-0">
                  <span className="shrink-0">{TIPO_ICON[tipoDetect]}</span>
                  <span className="text-xs font-bold text-[#13193a] bg-blue-50 border border-blue-200 rounded-full px-2 py-0.5 whitespace-nowrap">
                    {TIPO_LABEL[tipoDetect]}
                  </span>
                </div>
              )}
