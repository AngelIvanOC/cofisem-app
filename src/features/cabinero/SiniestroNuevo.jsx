...
const TIPO_LABEL = { poliza: "Póliza", serie: "No. Serie", placas: "Placas", reporte: "Reporte" };
...
const INP = "w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm text-gray-700 placeholder-gray-300 bg-white focus:outline-none focus:ring-2 focus:ring-[#13193a]/15 focus:border-[#13193a] transition-all";
    ...
    <div className="border border-gray-200 rounded-2xl overflow-hidden">
      ...
      <div className="flex items-center justify-between bg-gray-50 px-4 py-3 border-b border-gray-100">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-sm font-bold text-[#13193a]">Tercero / Afectado #{index + 1}</span>
        </div>
        <button
          type="button"
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <input value={tercero.conductorNombre} onChange={e => set("conductorNombre", e.target.value)} placeholder="Nombre del conductor" className={INP}/>
            </div>
            <div>
              <label className={LBL}>Teléfono</label>
              <input value={tercero.conductorTelefono} onChange={e => set("conductorTelefono", e.target.value)} placeholder="55 0000 0000" className={INP}/>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
            <div>
              ...
              <input value={tercero.vehiculoDesc} onChange={e => set("vehiculoDesc", e.target.value)} placeholder="Marca y modelo" className={INP}/>
            </div>
            <div>
              <label className={LBL}>Tipo</label>
              <input value={tercero.vehiculoTipo} onChange={e => set("vehiculoTipo", e.target.value)} placeholder="Automóvil, camión, moto..." className={INP}/>
            </div>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
            {[
              ...
              <div key={f.k}>
                ...
                <input value={tercero[f.k]} onChange={e => set(f.k, e.target.value)} placeholder={f.p} className={INP}/>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-[#13193a] px-6 py-5">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-white/60 text-sm mt-1.5 leading-snug max-w-sm">{poliza.titular}</p>
          </div>
      ...
      <div className="p-5 space-y-5">
        ...
        <div className="bg-gray-50 rounded-xl p-4 border border-gray-100">
          ...
          <div className="grid grid-cols-3 lg:grid-cols-5 gap-2">
            ...
            ].map(f => <DatoCard key={f.k} label={f.k} value={f.v} mono={f.k === "Serie"} />)}
          </div>
        </div>
...
function FormSiniestro({ poliza, onBack, onSubmit, loading }) {
  ...
  const [form, setForm] = useState({
    causa: "", circunstancia: "", detalles: "",
    entidad: "", conductorNA: "", telefono: "", ajustador: "",
  });
  const set = (k, v) => setForm(f => ({ ...f, [k]: v }));

  const [terceros, setTerceros] = useState([TERCERO_VACIO()]);
  const agregarTercero   = () => setTerceros(t => [...t, TERCERO_VACIO()]);
  const eliminarTercero  = (id) => { if (terceros.length > 1) setTerceros(t => t.filter(x => x.id !== id)); };
  const actualizarTercero = useCallback((id, campo, valor) => {
    setTerceros(t => t.map(x => x.id === id ? { ...x, [campo]: valor } : x));
  }, []);

  const terceroVacio = terceros.length === 1 && !terceros[0].conductorNombre && !terceros[0].vehiculoPlacas;
  ...
  return (
    <div className="space-y-4 pb-10">
      ...
      <Seccion titulo="Vehículo Asegurado">
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2">
            ...
            ].map(c => <DatoCard key={c.k} label={c.k} value={c.v} />)}
          </div>
        </div>
      </Seccion>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="bg-[#13193a] px-5 py-3.5 flex items-center justify-between">
          <div>
            <h3 className="text-sm font-bold text-white tracking-wide">Vehículos y Conductores Terceros / Afectados</h3>
        ...
        <div className="p-5 space-y-4">
          {terceroVacio && (
            <p className="text-xs text-gray-400 -mt-1 mb-1">
              Si no hay terceros involucrados, puedes dejar estos campos en blanco o eliminar la tarjeta.
            </p>
          )}
          {terceros.map((t, i) => (
            <TerceroCard
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <select value={form.causa} onChange={e => set("causa", e.target.value)} className={INP}>
            ...
            <div>
              ...
              <select value={form.circunstancia} onChange={e => set("circunstancia", e.target.value)} className={INP}>
      ...
      <Seccion titulo="Localización del Siniestro">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div>
            ...
            <select value={form.entidad} onChange={e => set("entidad", e.target.value)} className={INP}>
          ...
          <div>
            ...
            <input value={form.conductorNA} onChange={e => set("conductorNA", e.target.value)} placeholder="Nombre del conductor" className={INP}/>
          </div>
          <div>
            <label className={LBL}>Teléfono de contacto</label>
            <input value={form.telefono} onChange={e => set("telefono", e.target.value)} placeholder="55 0000 0000" className={INP}/>
          </div>
        </div>
      </Seccion>
      ...
      <Seccion titulo="Ajustador Asignado">
        <div className="max-w-xs">
          ...
          <select value={form.ajustador} onChange={e => set("ajustador", e.target.value)} className={INP}>
...
export default function SiniestroNuevo() {
  ...
  const handleBuscar = async () => {
    ...
    setBuscando(false);
    if (result) { setPoliza(result); setPaso("confirmar"); }
    else         { setNoEncontrado(true); inputRef.current?.select(); }
  };
  ...
  const resetBusqueda = () => {
    setPoliza(null); setQuery(""); setTipoDetect(null); setNoEncontrado(false); setPaso("buscar");
  };
  ...
  return (
    <div className="p-6 min-h-full bg-gray-50">
      ...
      <div className="flex items-start gap-4 mb-6">
        ...
        <div className="flex-1">
          ...
          <p className="text-gray-400 text-sm mt-0.5">
            {paso !== "formulario" ? "Paso 1 de 2 — Busca y confirma la póliza" : "Paso 2 de 2 — Completa los datos del siniestro"}
          </p>
        </div>
        ...
        <div className="hidden sm:flex items-center gap-2 shrink-0 mt-1">
          {[
            ...
            const isActive = (i === 0 && paso !== "formulario") || (i === 1 && paso === "formulario");
            return (
              <div key={s.n} className="flex items-center gap-2">
                {i > 0 && <div className={`w-8 h-px ${s.done || isActive ? "bg-[#13193a]" : "bg-gray-200"}`}/>}
                <div className="flex items-center gap-1.5">
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold transition-all ${
                    s.done ? "bg-emerald-500 text-white" :
                    isActive ? "bg-[#13193a] text-white" :
                  ...
                  <span className={`text-xs font-medium ${isActive ? "text-[#13193a]" : "text-gray-400"}`}>{s.label}</span>
                </div>
              </div>
            );
          })}
      ...
      {paso === "buscar" && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          ...
          <p className="text-xs text-gray-400 mb-5">
            Ingresa el <strong className="text-gray-600">número de póliza</strong>, <strong className="text-gray-600">No. de serie</strong>, <strong className="text-gray-600">placas</strong> o <strong className="text-gray-600">número de reporte</strong>. El sistema detecta automáticamente el tipo.
          </p>
            ...
            <div className="relative flex-1">
              ...
              <input
                ...
                className={[
                  ...
                  noEncontrado ? "border-red-300 focus:border-red-400" : "border-gray-200 focus:border-[#13193a]",
                ].join(" ")}
              />
            </div>
            <button
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {[
              ...
              <button
                key={h.label}
                onClick={() => { setQuery(h.ejemplo); setTipoDetect(detectarTipo(h.ejemplo)); setNoEncontrado(false); }}
      ...
      {paso === "confirmar" && poliza && (
        <PolizaCard poliza={poliza} onConfirmar={() => setPaso("formulario")} onLimpiar={resetBusqueda} />
      )}

      {/* ── Paso 2: Formulario ── */}
      {paso === "formulario" && poliza && (
        <FormSiniestro poliza={poliza} onBack={() => setPaso("confirmar")} onSubmit={handleSubmit} loading={loading} />
      )}
    </div>
  );
}