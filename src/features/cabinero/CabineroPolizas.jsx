...
import {
  actualizarNombreConcesionario,
  vincularConcesionario,
  fetchConcesionariosByCliente,
} from "../../services/concesionarios";
import ModalNuevoConcesionario from "../operador/components/ModalNuevoConcesionario";
...
function ModalEndoso({ poliza, onClose, onDone }) {
  ...
  const buildPreview = () => {
    ...
    switch (tipo) {
      ...
      case "Agregar concesionario": {
        const nuevoConc = concesionariosCliente.find(
          (c) => c.id === concesionarioSelId,
        );
        return nuevoConc
          ? buildDescripcionEndoso(tipo, null, nuevoConc.label)
          : "";
      }
      default:
        return "";
              ...
              <div>
                ...
                {!perms.asegurado && (
                  <p className="text-[10px] text-amber-600 mt-2">
                    Cambio de asegurado no disponible — el cliente tiene más de
                    1 póliza.
                  </p>
                )}
                {full.concesionarios && !perms.concesionario && (
                  <p className="text-[10px] text-amber-600 mt-1">
                    Cambio de concesionario no disponible — tiene más de 1
                    póliza.
                  </p>
                )}
              </div>
              ...
              {tipo === "Agregar concesionario" && (
                <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    >
                      <option value="">
                        {concesionariosCliente.length === 0
              ...
              {tipo === "Cambio de placas" && (
                <div>
                  ...
                  <input
                    value={form.placas}
                    onChange={(e) =>
                      set("placas", e.target.value.toUpperCase())
                    }
                    className={inpCls}
...
function ModalCambiarOficina({ poliza, onClose, onDone }) {
  ...
  const oficinaCambio =
    oficinaId !== "" &&
    Number(oficinaId) !==
      Number(poliza.oficina_id ?? poliza.oficinas?.id ?? null);
  const operadorFinal =
          ...
          {cargando ? (
            ...
            <>
              <div className="bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3 text-xs text-emerald-800 font-medium">
                Ingresa la fecha en que se recibió cada pago. Al guardar, la
                póliza cambiará a VIGENTE.
              </div>
              {cuotas.map((c) => (
                <div
                  key={c.id}
