...
// Paso 1: confirmar arribo + foto de llegada (se sube comprimida)
// ============================================================
import { useState, useRef } from "react";
import { subirEvidencia } from "../../services/evidencias";

export default function ConfirmarArribo({ siniestro, onConfirmar }) {
  const [confirmado,  setConfirmado]  = useState(false);
  const [fotoLocal,   setFotoLocal]   = useState(null);  // blob URL para preview
  ...
  const handleFoto = (e) => {
    ...
    // Subir en background inmediatamente
    setUploading(true);
    subirEvidencia({
  ...
  const handleConfirmar = async () => {
    if (uploading) return; // esperar a que termine la subida
    setConfirmado(true);
        ...
        {siniestro.ubicacion && (
          tieneUbic ? (
            ...
            <div className="flex items-center gap-2 p-3 rounded-xl bg-gray-50 border border-gray-100">
              ...
              <p className="text-xs text-gray-500 truncate">{siniestro.ubicacion}</p>
            </div>
          )
        )}

        {!siniestro.ubicacion && siniestro.telefono && (
          <a href={`tel:${siniestro.telefono}`}
            ...
            <div>
              ...
              <p className="text-[10px] text-amber-500">Sin ubicación — coordinar por teléfono</p>
            </div>
          </a>
        )}
      </div>
      ...
      <div>
        ...
        {fotoLocal ? (
          <div className="relative rounded-2xl overflow-hidden" style={{ height: 160 }}>
            ...
            <button
              onClick={() => { setFotoLocal(null); setUploadError(null); }}
