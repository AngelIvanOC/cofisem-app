...
// Acepta instanceKey para tener estado independiente por vehículo
// ============================================================
...
function SvgAuto({ vista }) {
  if (vista === "frente")
    return (
      <svg viewBox="0 0 200 140" className="w-full h-full" fill="none">
        ...
        />
        <path
          d="M55 60 Q60 25 90 22 H110 Q140 25 145 60Z"
          fill="#e2e8f0"
          stroke="#94a3b8"
          strokeWidth="2"
        />
        <path
          d="M60 59 Q65 30 88 28 H112 Q135 30 140 59Z"
          fill="#bfdbfe"
          stroke="#94a3b8"
          strokeWidth="1.5"
        />
  ...
  if (vista === "lateral_i" || vista === "lateral_d")
    return (
      <svg viewBox="0 0 260 140" className="w-full h-full" fill="none">
        ...
        />
        <path
          d="M40 72 Q55 30 90 26 H170 Q200 30 220 72Z"
          fill="#e2e8f0"
          stroke="#94a3b8"
          strokeWidth="2"
        />
        <path
          d="M48 71 Q62 34 91 30 H168 Q197 34 212 71Z"
          fill="#bfdbfe"
          stroke="#94a3b8"
          strokeWidth="1.5"
        />
...
export default function ModeloDanos({ instanceKey = "default" }) {
  ...
  const handleClickSvg = (e) => {
    ...
    setPuntosMap((prev) => ({
      ...prev,
      [vistaActual]: [...(prev[vistaActual] ?? []), { id, x, y, nota: "" }],
    }));
    setPuntoActivo(id);
    setNotaTemp("");
  };
      ...
      >
        ...
        {puntosVista.length === 0 && (
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
            <p className="text-[11px] text-gray-400 font-semibold bg-white/80 px-3 py-1.5 rounded-xl border border-gray-200">
              Toca para marcar zona dañada
            </p>
          </div>
        )}
          ...
          >
            <div className="relative">
              ...
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  setPuntoActivo(p.id === puntoActivo ? null : p.id);
                  setNotaTemp(p.nota);
                }}
                className={[
      ...
      {puntoActivo !== null && (
        <div className="bg-white rounded-2xl border-2 border-[#13193a]/20 p-4 space-y-3">
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={guardarNota}
              className="flex-1 py-2.5 rounded-xl bg-[#13193a] text-white text-xs font-bold"
            >
              Guardar nota
            </button>
          </div>
      ...
      {totalPuntos > 0 && (
        <div className="bg-red-50 border border-red-100 rounded-xl p-3">
          <p className="text-[11px] font-bold text-red-600 uppercase tracking-widest mb-2">
            {totalPuntos} zona{totalPuntos > 1 ? "s" : ""} marcada
            {totalPuntos > 1 ? "s" : ""}
          </p>
          <div className="space-y-1.5">
            {VISTAS_AUTO.filter((v) => (puntosMap[v.id]?.length ?? 0) > 0).map(
              (v) => (
                <div key={v.id}>
                  ...
                  {puntosMap[v.id].map((p, i) => (
                    <div key={p.id} className="flex items-start gap-2 mt-1">
                      ...
                      <p className="text-xs text-gray-600 leading-snug">
                        {p.nota || (
                          <span className="italic text-gray-400">Sin nota</span>
                        )}
                      </p>
                    </div>
                  ))}
