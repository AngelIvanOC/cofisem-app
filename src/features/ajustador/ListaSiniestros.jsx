...
// Mobile: fluye en <main> del AppLayout — sin h-full propio
// Desktop (prop `desktop`): usa flex-col h-full con overflow-y-auto
// ============================================================
import { useState } from "react";
import { SINIESTROS_MOCK, METRICAS, ESTATUS_CLS } from "./shared";

export default function ListaSiniestros({ onAtender, desktop = false }) {
  ...
  const Header = () => (
    <div
      className={
        desktop
          ? "px-4 pt-5 pb-4 border-b border-gray-100 shrink-0"
          : "px-4 pt-5 pb-4 border-b border-gray-100"
      }
    >
      ...
      <div className="flex gap-2 mt-3">
        {METRICAS.map((m) => (
          <div
            key={m.label}
            className={`flex-1 rounded-xl p-3 ${m.bg} border border-white`}
          >
  ...
  if (desktop) {
    // Desktop: contenedor con altura fija y overflow interno
    return (
  ...
  // Mobile: fluye libremente en el <main> del AppLayout
  return (
    <div>
      <Header />
      <Cards />
