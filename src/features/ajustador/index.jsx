...
// Fix mobile scroll + paso 2 unificado (datos + evidencia + mapa por vehículo)
// Pasos: 0-Arribo | 1-Datos+Evidencia+Mapa | 2-Documentos
// ============================================================
...
export default function AjustadorSiniestros() {
  ...
  return (
    // Fix scroll mobile: h-full en lugar de min-h-screen, overflow correcto
      ...
      {vista === "detalle" && siniestro && (
        <div className="flex-1 flex flex-col bg-white min-h-0 overflow-hidden">
          ...
          <div className="px-4 py-3 border-b border-gray-100 bg-gray-50/80 shrink-0">
            <StepBar
              paso={paso}
              labels={["Arribo", "Datos y Evidencia", "Documentos"]}
            />
          </div>

          {/* Contenido — scroll habilitado correctamente */}
          <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
            {paso === 0 && (
              <ConfirmarArribo
                siniestro={siniestro}
                onConfirmar={() => setPaso(1)}
              />
            )}
            {paso === 1 && (
      ...
      {vista === "lista" && (
        <div className="hidden md:flex flex-1 flex-col items-center justify-center bg-gray-50 text-center p-8">
          ...
          <p className="text-gray-400 text-xs mt-1 max-w-xs">
            Elige un caso de la lista para comenzar el proceso de atención.
          </p>
        </div>
      )}
    </div>
