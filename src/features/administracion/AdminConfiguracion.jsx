...
import {
  ...
  setPermitirFechasPasadas,
  fetchPermitirNumeroManual,
  setPermitirNumeroManual,
...
function ModalNuevoCosto({ onClose, onGuardado, usuario }) {
  ...
  const handleGuardar = async () => {
    ...
    try {
      const rows = [
        {
          clave: "derechos_emision",
          valor: String(derechos),
          tipo: "number",
          descripcion: "Derechos de emisión por póliza (MXN)",
          vigente_desde: desde,
        ...
        },
        {
          ...
          valor: String(iva),
          tipo: "percent",
          descripcion: "Porcentaje de IVA aplicado a la prima",
          vigente_desde: desde,
          updated_by: usuario?.id ?? null,
        },
      ];
      ...
      >
        <div className="flex items-center gap-3 px-6 py-4 border-b border-gray-100">
          ...
          <div className="flex-1">
            <h2 className="text-sm font-bold text-[#13193a]">
              Programar nuevos costos
            </h2>
            <p className="text-xs text-gray-400 mt-0.5">
              Ingresa ambos valores y la fecha de entrada en vigor
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-4">
          <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-xs text-amber-800">
            Solo afectará a pólizas emitidas a partir de la fecha seleccionada.
            Las pólizas anteriores conservan sus valores originales.
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[11px] font-semibold text-gray-400 uppercase tracking-wide mb-1.5">
                Derechos de emisión (MXN){" "}
          ...
          {derechos && iva && (
            <div className="bg-gray-50 rounded-xl p-4 border border-gray-100 space-y-1.5">
              ...
              {[
                ...
                [
                  "+ IVA " + iva + "%",
                  (((1755.17 + Number(derechos)) * Number(iva)) / 100).toFixed(
                    2,
                  ),
                ],
              ].map(([l, v]) => (
                <div key={l} className="flex justify-between text-xs">
                  ...
                  <span className="font-mono font-semibold text-[#13193a]">
                    ${v}
                  </span>
                </div>
              ))}
  ...
  const handleGuardar = async () => {
    ...
    try {
      const { error: e1 } = await supabase
        .from("coberturas")
        .update({ activa: false, activa_hasta: desde })
        .eq("id", cobertura.id);
      if (e1) throw e1;
      const { data: nueva, error: e2 } = await supabase
        ...
        .insert({
          ...
          grupo_id: cobertura.grupo_id,
          oficina_id: cobertura.oficina_id ?? null,
          duracion_meses: cobertura.duracion_meses,
      ...
      const { data: rubros } = await supabase
        .from("cobertura_rubros")
        .select("rubro, prima_neta, monto_maximo, es_sublimite, orden")
        .eq("cobertura_id", cobertura.id);
      if (rubros?.length)
        await supabase
          .from("cobertura_rubros")
          .insert(rubros.map((r) => ({ ...r, cobertura_id: nueva.id })));
      onGuardado();
      Swal.fire({
        icon: "success",
        title: "Nueva versión creada",
        text: `Nuevos precios vigentes desde ${fmtFecha(desde)}.`,
        confirmButtonColor: "#13193a",
        timer: 5000,
        timerProgressBar: true,
      });
    ...
    >
      ...
      >
        <div className="flex items-center gap-3 px-6 py-4 border-b border-gray-100">
          ...
          <div className="flex-1">
            <h2 className="text-sm font-bold text-[#13193a]">
              Nueva versión de cobertura
            </h2>
            <p className="text-xs text-gray-400 mt-0.5 truncate max-w-xs font-mono">
              {cobertura.nombre}
            </p>
          </div>
          <button
        ...
        <div className="p-6 space-y-4">
          <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-xs text-amber-800">
            Solo afecta pólizas emitidas a partir de la fecha elegida. Los
            rubros se copian automáticamente.
          </div>
          <div className="grid grid-cols-2 gap-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[11px] font-semibold text-gray-400 uppercase tracking-wide mb-1.5">
                Nueva prima neta <span className="text-red-400">*</span>
              </label>
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm font-medium">
                  $
            ...
            <div>
              <label className="block text-[11px] font-semibold text-gray-400 uppercase tracking-wide mb-1.5">
                Nueva prima total <span className="text-red-400">*</span>
              </label>
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm font-medium">
                  $
...
function ModalOficina({ oficina, onClose, onGuardado }) {
  ...
  const [color, setColor] = useState(
    esHexValido(oficina?.color) ? oficina.color : "#2a78d6",
  );
  const [guardando, setGuardando] = useState(false);
      ...
      >
        <div className="flex items-center gap-3 px-6 py-4 border-b border-gray-100">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              El color identifica a la oficina en gráficas y tablas de Detalle
              de Primas
            </p>
          </div>
          <button
...
function SeccionMiPassword() {
  ...
  const valido =
    passActual && passNueva.length >= 6 && passNueva === passConfirmar;
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-5">
      ...
      <p className="text-xs text-gray-400 mb-4 leading-relaxed">
        Al guardar, se cerrará tu sesión y deberás iniciar de nuevo con la nueva
        contraseña.
      </p>
...
function SeccionPasswordUsuario() {
  ...
  const valido =
    usuarioId && passNueva.length >= 6 && passNueva === passConfirmar;
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-5">
      ...
      <p className="text-xs text-gray-400 mb-4 leading-relaxed">
        Se actualiza de inmediato; el usuario deberá usar la nueva contraseña en
...
export default function AdminConfiguracion({ usuario }) {
  ...
  const cargarCostos = useCallback(async () => {
    ...
    const { data } = await supabase
      .from("configuracion_costos")
      .select("clave, valor, tipo, descripcion, vigente_desde, updated_at")
  ...
  const cargarCoberturas = useCallback(async () => {
    ...
    const [{ data: cobs }, { data: grps }] = await Promise.all([
      supabase
        .from("coberturas")
        .select(
          "*, grupos_coberturas(nombre), cobertura_rubros(id, rubro, prima_neta, monto_maximo, es_sublimite, orden)",
        )
        .order("nombre"),
  ...
  for (const row of histCostos) {
    if (row.vigente_desde <= h && !(row.clave in currentCostos))
      currentCostos[row.clave] = row;
  }
  const derechosActual = currentCostos["derechos_emision"];
  const ivaActual = currentCostos["iva_pct"];

  const activasPorGrupo = grupos
    .map((g) => ({
      grupo: g,
    ...
    <div className="p-6 min-h-full bg-gray-50 space-y-5">
      <div>
        <h1 className="text-2xl font-bold text-[#13193a]">
          Ajustes, precios y coberturas
        </h1>
        <p className="text-gray-400 text-sm mt-0.5">
          Los cambios solo aplican a pólizas emitidas después de la fecha
          programada
        </p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="flex items-center border-b border-gray-100 px-2">
          {[
            { k: "costos", l: "Costos de emisión" },
            { k: "coberturas", l: "Coberturas" },
            { k: "oficinas", l: "Oficinas" },
            { k: "sistema", l: "Sistema" },
                  ...
                  <div className="rounded-2xl border border-gray-100 p-5 bg-white">
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <p className="text-3xl font-bold text-[#13193a] mt-1 tabular-nums">
                          {derechosActual ? fmt$(derechosActual.valor) : "—"}
                        </p>
                        <p className="text-xs text-gray-400 mt-1">
                          Vigente desde{" "}
                          {derechosActual
                            ? fmtFecha(derechosActual.vigente_desde)
                            : "—"}
                        </p>
                      </div>
                  ...
                  <div className="rounded-2xl border border-gray-100 p-5 bg-white">
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <p className="text-3xl font-bold text-[#13193a] mt-1 tabular-nums">
                          {ivaActual ? `${ivaActual.valor}%` : "—"}
                        </p>
                        <p className="text-xs text-gray-400 mt-1">
                          Vigente desde{" "}
                          {ivaActual ? fmtFecha(ivaActual.vigente_desde) : "—"}
                        </p>
                      </div>
                ...
                {activasPorGrupo.map(({ grupo, items }) => (
                  <div key={grupo.id}>
                    ...
                    <div className="space-y-2">
                      {items.map((cob) => {
                        const rubros = (cob.cobertura_rubros ?? []).sort(
                          (a, b) => a.orden - b.orden,
                        );
                          ...
                          >
                            <div className="flex items-center gap-4 px-5 py-3.5">
                              <div className="flex-1 min-w-0">
                                <p className="text-sm font-semibold text-[#13193a] truncate">
                                  {cob.nombre}
                                </p>
                                <p className="text-xs text-gray-400 mt-0.5">
                                  {cob.duracion_meses === 6
                                    ? "6 meses"
                                    : "Anual"}{" "}
                                  · Vigente desde {fmtFecha(cob.vigente_desde)}
                              ...
                              <div className="flex items-center gap-6 shrink-0">
                                <div className="text-right">
                                  ...
                                  <p className="text-sm font-bold text-[#13193a] tabular-nums">
                                    {fmt$(cob.prima_neta)}
                                  </p>
                                </div>
                                <div className="text-right">
                                  ...
                                  <tbody className="divide-y divide-gray-100">
                                    {rubros.map((r) => (
                                      <tr
                                        key={r.id}
                                        className={
                                          r.es_sublimite ? "opacity-60" : ""
                                        }
                                      >
                                        <td
                                          className={`py-1.5 text-gray-700 ${r.es_sublimite ? "pl-4" : "font-medium"}`}
                                        >
                                          {r.es_sublimite ? "↳ " : ""}
                                          {r.rubro}
                                        </td>
                                        ...
                                        <td className="py-1.5 text-right font-mono text-[#13193a]">
                                          {r.prima_neta > 0
                                            ? fmt$(r.prima_neta)
                                            : "—"}
                                        </td>
                            ...
                            {abierto && rubros.length === 0 && (
                              <div className="border-t border-gray-100 bg-gray-50/60 px-5 py-4 text-xs text-gray-400 text-center">
                        ...
                        <tbody className="divide-y divide-gray-50">
                          {inactivas.map((c) => (
                            ...
                            >
                              ...
                              <td className="px-4 py-2.5 text-xs text-gray-500">
                                {fmtFecha(c.vigente_desde)}
                              </td>
                              <td className="px-4 py-2.5 text-xs text-gray-500">
                                {fmtFecha(c.activa_hasta)}
                              </td>
                            </tr>
                          ))}
                        </tbody>
            ...
            {cargandoOfs ? (
              ...
              <div className="overflow-x-auto rounded-xl border border-gray-100">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-50/80 border-b border-gray-100">
                      <th className="text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wide px-4 py-2.5 w-16">
                        Color
                      </th>
                      <th className="text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wide px-4 py-2.5">
                        Oficina
                      </th>
                      <th className="text-right text-[11px] font-semibold text-gray-400 uppercase tracking-wide px-4 py-2.5 w-28">
                        Acciones
                      </th>
                    </tr>
                  </thead>
                      ...
                      <tr key={of_.id} className="hover:bg-gray-50/60">
                        <td className="px-4 py-2.5">
                          <span
                            ...
                            style={{
                              background: esHexValido(of_.color)
                                ? of_.color
                                : "#e5e7eb",
                            }}
              ...
              <div className="flex items-start justify-between bg-white rounded-2xl border border-gray-100 px-5 py-4 gap-6 mb-3">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <p className="text-sm font-bold text-[#13193a]">
                      Permitir número de póliza manual
                    </p>
                  </div>
                  <p className="text-xs text-gray-400 leading-relaxed">
                    ...
                    , el operador puede escribir manualmente el número de
                    constancia al cotizar. El sistema verifica que no esté
                    duplicado. Si se deja vacío, el sistema lo asigna
                    automáticamente.
                  </p>
                  {permitirNumMan && (
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      Swal.fire({
                        icon: "success",
                        title: nuevo
                          ? "Número manual habilitado"
                          : "Número manual deshabilitado",
                        text: nuevo
                ...
                >
                  {guardandoSist ? (
                    ...
                    <span
                      className={`inline-block h-5 w-5 transform rounded-full bg-white shadow transition-transform duration-200 ${permitirNumMan ? "translate-x-[26px]" : "translate-x-[3px]"}`}
                    />
                  )}
                </button>
              </div>
              ...
              <div className="flex items-start justify-between bg-white rounded-2xl border border-gray-100 px-5 py-4 gap-6">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <Settings className="w-4 h-4 text-[#13193a]" />
                    <p className="text-sm font-bold text-[#13193a]">
                      Permitir fechas de inicio anteriores a hoy
                    </p>
                  </div>
                  <p className="text-xs text-gray-400 leading-relaxed">
                    Cuando está{" "}
                    ...
                    , el operador puede emitir pólizas con cualquier fecha de
                    inicio (incluidas fechas pasadas). Cuando está{" "}
                    <span className="font-semibold">desactivado</span> (por
                    defecto), solo se permiten fechas desde hoy hasta 30 días en
                    el futuro.
                  </p>
                  {permitirFechas && (
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    setGuardandoSist(true);
                    try {
                      await setPermitirFechasPasadas(
                        nuevo,
                        usuario?.id ?? null,
                      );
                ...
                >
                  {guardandoSist ? (
                    ...
                    <span
                      className={`inline-block h-5 w-5 transform rounded-full bg-white shadow transition-transform duration-200 ${permitirFechas ? "translate-x-[26px]" : "translate-x-[3px]"}`}
                    />
                  )}
                </button>
              </div>
      ...
      {cobModal && (
        <ModalNuevaVersionCobertura
          cobertura={cobModal}
          onClose={() => setCobModal(null)}
          onGuardado={() => {
            setCobModal(null);
            cargarCoberturas();
          }}
      ...
      {modalOficina && (
        <ModalOficina
          oficina={oficinaEdit}
          onClose={() => setModalOficina(false)}
          onGuardado={cargarOficinas}
        />
      )}
    </div>
  );
