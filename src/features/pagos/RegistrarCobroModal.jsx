...
// particular (mismo desglose que la cuota 1: efectivo/cheque/tdc/vale/
// pól. pend. pago + comprobantes + autorización), para que el efectivo
...
import { subirComprobantePago, verComprobantePago, MAX_PAGO_COMPROBANTE_BYTES } from "../../services/comprobantesPagoCofisem";
...
const VACIO = {
  prima_total: "", prima_neta: "", fecha_recibido: "",
  vale: "", efectivo: "", cheque: "", tdc: "", pol_pend_pago: "", autorizacion: "",
  comprobante_vale_path: null, comprobante_cheque_path: null, comprobante_tdc_path: null,
};
...
export default function RegistrarCobroModal({ row, usuario, onClose, onSaved, onPerdida }) {
  const [form, setForm] = useState({ ...VACIO });
  const [subiendo, setSubiendo] = useState(null); // 'vale' | 'cheque' | 'tdc' | null
  ...
  const campoCls = (k) => `${inp}${falta(k) ? " border-red-300 ring-2 ring-red-100" : ""}`;
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="px-6 py-4 border-b border-gray-100 sticky top-0 bg-white rounded-t-2xl">
          <p className="text-sm font-bold text-[#1447e6]">Registrar cobro de cuota</p>
          <p className="text-xs text-gray-400 mt-0.5">
            Póliza <strong className="font-mono">{p.numero_poliza || "—"}</strong> · {p.asegurado_nombre || "—"} · Cuota <strong>{row.num_cuota}</strong>
          </p>
        </div>
        ...
        <form onSubmit={guardar} className="p-6 space-y-4">
          ...
          <div className="rounded-xl bg-gray-50 border border-gray-100 px-4 py-3 flex items-center gap-2 text-[11px] text-gray-500">
            ...
            Los datos de la póliza ({p.aseguradora || "—"}, {p.forma_pago || "—"}) ya están registrados — aquí solo se captura cómo se pagó esta cuota.
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className={lbl}>Prima Total de la cuota</label>
              <input type="number" min="0" step="0.01" value={form.prima_total} onChange={(e) => setF("prima_total", e.target.value)} className={inp} />
            </div>
            <div>
              <label className={lbl}>Prima Neta de la cuota</label>
              <input type="number" min="0" step="0.01" value={form.prima_neta} onChange={(e) => setF("prima_neta", e.target.value)} className={inp} />
            </div>
          </div>

          <div>
            <label className={lbl}>Fecha en que se recibió el pago</label>
            <input type="date" value={form.fecha_recibido} onChange={(e) => setF("fecha_recibido", e.target.value)} className={inp + " sm:max-w-xs"} />
          </div>

          <div>
            <p className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-3">
              Forma de pago {falta("pago") && <span className="text-red-500 normal-case tracking-normal ml-1">— marca al menos una</span>}
            </p>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <div>
                <label className={lbl}>Vale ($)</label>
                <input type="number" min="0" step="0.01" value={form.vale} onChange={(e) => setF("vale", e.target.value)} placeholder="0.00" className={inp} />
              </div>
              <div>
                <label className={lbl}>Efectivo</label>
                <input type="number" min="0" step="0.01" value={form.efectivo} onChange={(e) => setF("efectivo", e.target.value)} placeholder="0.00" className={campoCls("pago")} />
              </div>
              <div>
                <label className={lbl}>Cheque / Dep.</label>
                <input type="number" min="0" step="0.01" value={form.cheque} onChange={(e) => setF("cheque", e.target.value)} placeholder="0.00" className={campoCls("pago")} />
              </div>
              <div>
                <label className={lbl}>T. Crédito/Déb.</label>
                <input type="number" min="0" step="0.01" value={form.tdc} onChange={(e) => setF("tdc", e.target.value)} placeholder="0.00" className={campoCls("pago")} />
              </div>
              <div>
                <label className={lbl}>Pól. Pend. Pago</label>
                <input type="number" min="0" step="0.01" value={form.pol_pend_pago} onChange={(e) => setF("pol_pend_pago", e.target.value)} placeholder="0.00" className={campoCls("pago")} />
              </div>
            </div>
            {n(form.tdc) > 0 && (
              <div className="mt-4">
                <label className={lbl}>Autorización <span className={falta("autorizacion") ? "text-red-500" : "text-gray-300"}>*</span></label>
            ...
            {(n(form.vale) > 0 || n(form.cheque) > 0 || n(form.tdc) > 0) && (
              <div className="mt-3 space-y-2">
                {n(form.vale) > 0 && (
                  <ComprobanteField label="Comprobante del vale (foto del papel)" path={form.comprobante_vale_path} subiendo={subiendo === "vale"} onFile={(f) => handleArchivo("vale", f)} onVer={() => verComprobantePago(form.comprobante_vale_path)} />
                )}
                {n(form.cheque) > 0 && (
                  <ComprobanteField label="Comprobante de cheque / depósito / transferencia" path={form.comprobante_cheque_path} subiendo={subiendo === "cheque"} onFile={(f) => handleArchivo("cheque", f)} onVer={() => verComprobantePago(form.comprobante_cheque_path)} />
                )}
                {n(form.tdc) > 0 && (
                  <ComprobanteField label="Comprobante de la terminal (ticket TDC)" path={form.comprobante_tdc_path} subiendo={subiendo === "tdc"} onFile={(f) => handleArchivo("tdc", f)} onVer={() => verComprobantePago(form.comprobante_tdc_path)} />
                )}
              </div>
            )}
          </div>
          ...
          <div className="flex items-center justify-end gap-3 pt-2 border-t border-gray-100">
            ...
            <button type="submit" disabled={guardando} className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-[#1447e6] hover:bg-[#0f36b3] text-white text-sm font-bold disabled:opacity-50">
          ...
          <div className="pt-1">
            {!perdidaAbierto ? (
              <button type="button" onClick={() => setPerdidaAbierto(true)} className="text-[11px] font-semibold text-gray-400 hover:text-red-500 underline underline-offset-2">
              ...
              <div className="rounded-xl border border-red-200 bg-red-50/60 p-4 space-y-3">
                <p className="text-xs font-bold text-red-700">
                  Esto da por perdida TODA la póliza ({p.numero_poliza || "—"}), no solo esta cuota — ya no se podrá registrar ningún cobro de sus demás cuotas, y dejará de aparecer en Pólizas del día. Se sigue mostrando en Pagos como registro.
                </p>
                <textarea
                  value={perdidaNota}
                ...
                <div className="flex items-center justify-end gap-2">
                  <button type="button" onClick={() => setPerdidaAbierto(false)} className="px-4 py-2 rounded-lg border border-gray-200 text-xs font-semibold text-gray-600 hover:bg-white">
                    Cancelar
                  </button>
                  <button type="button" onClick={confirmarPerdida} disabled={dandoBaja} className="px-4 py-2 rounded-lg bg-red-600 hover:bg-red-700 text-white text-xs font-bold disabled:opacity-50">
