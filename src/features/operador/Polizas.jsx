            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2">
                <CampoModal
                  ...
                  value={form.calle}
                  onChange={(v) => setF("calle", v)}
                  placeholder="Av. Emiliano Zapata 145"
                />
              </div>
              <CampoModal
                label="Colonia"
                value={form.colonia}
                onChange={(v) => setF("colonia", v)}
                placeholder="Colonia"
              />
              <CampoModal
                label="Municipio"
                value={form.municipio}
                onChange={(v) => setF("municipio", v)}
                placeholder="Municipio"
              />
              <CampoModal
                label="C.P."
                value={form.cp}
                onChange={(v) => setF("cp", v)}
                placeholder="62000"
              />
              <div>
                <label className="block text-[11px] font-bold text-gray-400 uppercase tracking-wide mb-1.5">
...
function FormCotizacion({
  ...
  // Cascada de vehículo
  const tiposDisponibles = form.marca
    ? Object.keys(CATALOGO_VEHICULOS[form.marca])
    : [];
  const versionesDisponibles =
    form.marca && form.tipo ? CATALOGO_VEHICULOS[form.marca][form.tipo] : [];
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <select
              ...
              disabled={!form.marca}
              className={
                inpCls + (form.marca ? "" : " opacity-50 cursor-not-allowed")
              }
            >
              <option value="">
          ...
          <div>
            ...
            <select
              ...
              disabled={!form.tipo}
              className={
                inpCls + (form.tipo ? "" : " opacity-50 cursor-not-allowed")
              }
            >
              <option value="">
