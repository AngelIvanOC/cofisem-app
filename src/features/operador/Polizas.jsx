...
export default function Polizas({ usuario }) {
  ...
  const polizasFiltradas = useMemo(() => {
    ...
    return polizas.filter((p) => {
      ...
      const matchBusq =
        asegurado.includes(b) ||
        constancia.includes(b) ||
        numPoliza.includes(b);
        ...
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
            {(() => {
              ...
              const enMesISO = (iso, anio, mes) => {
                if (!iso) return false;
                const d = new Date(iso + "T12:00:00");
                return d.getFullYear() === anio && d.getMonth() + 1 === mes;
              };
              const isRenovada = (p) => {
                const m = (p.constancia || p.numero_poliza || "").match(
                  /-(\d+)$/,
                );
                return m && parseInt(m[1]) >= 2;
              };
              const activas = (p) =>
                ["VIGENTE", "POR VENCER"].includes(p.estatus);
              const cards = [
                {
                  label: "Emitidas",
                  count: polizas.filter(activas).length,
                  curr: polizas.filter(
                    (p) =>
                      activas(p) && enMesISO(p.fecha_inicio, anioAct, mesAct),
                  ).length,
                  prev: polizas.filter(
                    (p) =>
                      activas(p) && enMesISO(p.fecha_inicio, anioPrev, mesPrev),
                  ).length,
                  positivo: true,
                },
                {
                  label: "Renovadas",
                  count: polizas.filter((p) => isRenovada(p) && activas(p))
                    .length,
                  curr: polizas.filter(
                    (p) =>
                      isRenovada(p) &&
                      activas(p) &&
                      enMesISO(p.fecha_inicio, anioAct, mesAct),
                  ).length,
                  prev: polizas.filter(
                    (p) =>
                      isRenovada(p) &&
                      activas(p) &&
                      enMesISO(p.fecha_inicio, anioPrev, mesPrev),
                  ).length,
                  positivo: true,
                },
                {
                  label: "Vencidas",
                  count: polizas.filter((p) => p.estatus === "VENCIDA").length,
                  curr: polizas.filter(
                    (p) =>
                      p.estatus === "VENCIDA" &&
                      enMesISO(p.fecha_fin, anioAct, mesAct),
                  ).length,
                  prev: polizas.filter(
                    (p) =>
                      p.estatus === "VENCIDA" &&
                      enMesISO(p.fecha_fin, anioPrev, mesPrev),
                  ).length,
                  positivo: false,
                },
                {
                  label: "Canceladas",
                  count: polizas.filter((p) =>
              ...
              return cards.map((m) => {
                ...
                const igual = delta === 0;
                const esBueno =
                  sinRef || igual ? null : m.positivo ? sube : !sube;
                const color =
                  esBueno === null
                ...
                const Icono =
                  sinRef || igual ? Minus : sube ? TrendingUp : TrendingDown;
                const texto = sinRef
                  ? "sin ref."
                  : `${pct > 0 ? "+" : ""}${pct}%`;
                return (
                  ...
                  >
                    ...
                    <div className="flex items-end justify-between gap-2">
                      ...
                      <span
                        className={`shrink-0 px-2.5 py-1.5 rounded-xl border text-xs font-bold tabular-nums flex items-center gap-1.5 ${color}`}
                      >
                    ...
                    <tbody className="divide-y divide-gray-50">
                      {subsecuentes.map((s) => (
                        ...
                        >
                          ...
                          <td className="px-5 py-2 text-xs text-gray-500 max-w-[10rem] truncate">
                            {s.coberturas?.nombre ?? "—"}
                          </td>
