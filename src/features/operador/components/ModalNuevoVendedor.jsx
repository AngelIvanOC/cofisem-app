...
export default function ModalNuevoVendedor({ onClose, onGuardar, usuarioId }) {
  ...
  const guardar = async () => {
    ...
    setSaving(true);
    try {
      const vendedor = await crearVendedor({ ...form, oficina: OFICINA.codigo }, usuarioId);
      onGuardar(vendedor);
    } catch (e) {
    ...
    >
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div>
            <h2 className="text-base font-bold text-[#13193a]">Nuevo vendedor</h2>
            <p className="text-xs text-gray-400 mt-0.5">El número se asigna automáticamente</p>
          </div>
        ...
        <div className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <Campo label="Nombre(s)" value={form.nombre}   onChange={v => setF("nombre", v)}   placeholder="Nombre" req />
