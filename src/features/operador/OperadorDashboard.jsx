    ...
    <div className="h-full overflow-y-auto bg-[#f7f8fa]">
      <div className="max-w-7xl mx-auto p-6 space-y-5">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { label: "Pólizas emitidas hoy",    value: "8",      delta: "+2 vs ayer",        up: true,  spark: SPARK_POLIZAS,           color: "#13193a", path: "/polizas"       },
            { label: "Cobrado hoy",              value: "$13,573",delta: "+$2,100 vs ayer",    up: true,  spark: SPARK_COBRO,             color: "#059669", path: "/corte-diario"  },
            { label: "Cotizaciones guardadas",   value: "3",      delta: "Pendientes",         up: null,  spark: [1, 2, 3, 2, 4, 3, 3],  color: "#d97706", path: "/cotizaciones"  },
            { label: "Pólizas por vencer",       value: "5",      delta: "Próximos 7 días",    up: false, spark: [2, 1, 3, 2, 4, 3, 5],  color: "#ef4444", path: "/polizas"       },
          ].map((k) => (
            <button
              key={k.label}
            ...
            >
              <div className="flex items-start justify-between mb-3">
                <p className="text-xs text-gray-400 font-medium leading-snug">{k.label}</p>
              ...
              <p className="text-2xl font-bold text-[#13193a] tabular-nums mb-2">{k.value}</p>
              <SparkBar data={k.spark} color={k.color} />
              <p className={`text-[11px] font-medium mt-2 flex items-center gap-1 ${
                k.up === true ? "text-emerald-600" : k.up === false ? "text-red-500" : "text-amber-600"
              }`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
              ...
              {ULTIMAS.map((p, i) => (
                <div key={i} className="flex items-center gap-3 px-5 py-3 hover:bg-gray-50/60 transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[11px] text-gray-400 truncate">{p.cobertura} · {p.hora}</p>
                  </div>
                  <div className="text-right shrink-0">
                    <p className="text-xs font-bold text-emerald-700">{p.prima}</p>
                    <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${STATUS[p.estatus]}`}>
                      {p.estatus}
                    </span>
                  </div>
                </div>
          ...
          <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-100 overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-50">
              <p className="text-sm font-bold text-[#13193a]">Cotizaciones guardadas</p>
              <button onClick={() => navigate("/cotizaciones")} className="text-xs text-blue-500 font-semibold hover:underline">
                Ver todas
              </button>
            </div>
            <div className="divide-y divide-gray-50">
              {COTIZACIONES_DASH.map((c, i) => (
                <div key={i} className="px-5 py-3 hover:bg-gray-50/60 transition-colors">
                  <div className="flex items-center justify-between mb-1">
                    <p className="text-[11px] font-mono font-bold text-[#13193a]">{c.id}</p>
                    <p className="text-xs font-bold text-emerald-700">{c.prima}</p>
                  </div>
                  <p className="text-xs font-medium text-gray-700">{c.cliente}</p>
                  <div className="flex items-center justify-between mt-1.5">
                    <p className="text-[10px] text-gray-400 truncate">{c.cobertura}</p>
                    <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="bg-white rounded-2xl border border-amber-100 overflow-hidden">
            <div className="flex items-center gap-2 px-5 py-4 border-b border-amber-50 bg-amber-50/50">
              ...
              <p className="text-sm font-bold text-amber-800">Próximas a vencer</p>
            </div>
            <div className="divide-y divide-amber-50/50">
              {POR_VENCER.map((p, i) => (
                <div key={i} className="flex items-center justify-between px-5 py-3">
                  <div>
                    <p className="text-xs font-semibold text-[#13193a]">{p.asegurado}</p>
                    <p className="text-[10px] font-mono text-gray-400">{p.poliza}</p>
                  </div>
                  <span className={`text-[11px] font-bold px-2.5 py-1 rounded-full ${
                    p.dias <= 3 ? "bg-red-100 text-red-700" : "bg-amber-100 text-amber-700"
                  }`}>
                    {p.dias}d
                  </span>
                </div>
          ...
          <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-100 p-5">
            <p className="text-sm font-bold text-[#13193a] mb-4">Resumen de cobro — hoy</p>
            <div className="grid grid-cols-3 gap-3">
              {[
                { label: "Efectivo",       value: "$10,893", pct: 80, color: "#13193a" },
                { label: "Transferencias", value: "$1,780",  pct: 13, color: "#3b82f6" },
                ...
                <div key={f.label} className="text-center">
                  <div className="relative w-16 h-16 mx-auto mb-2">
                    <svg className="w-full h-full -rotate-90" viewBox="0 0 36 36">
                      ...
                      <circle cx="18" cy="18" r="14" fill="none" stroke={f.color} strokeWidth="3.5"
                        strokeDasharray={`${f.pct * 0.879} 87.9`} strokeLinecap="round" />
                    </svg>
            ...
            <div className="mt-4 pt-4 border-t border-gray-100 flex items-center justify-between">
              ...
              <p className="text-lg font-black text-[#13193a] tabular-nums">$13,573</p>
            </div>
          </div>
        </div>
      </div>
