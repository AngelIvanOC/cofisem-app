...
function CuotaBadge({ cuota }) {
  ...
  return (
    <span
      className={`text-[11px] font-bold px-2.5 py-1 rounded-full border ${map[est]}`}
    >
      {labels[est]}
    </span>
  );
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              Cuota {cuota.num} · Póliza{" "}
              <span className="font-mono">{poliza.id}</span>
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-4">
          ...
          <p className="text-[11px] text-gray-400 text-center">
            El pago quedará como <strong>ADEUDO</strong> hasta que el analista
            lo confirme.
          </p>
        </div>
        ...
        <div className="flex gap-3 px-6 pb-6">
          ...
          <button
            ...
            disabled={aplicando || !monto || !fecha}
...
function ModalHistorial({ poliza, onClose, onAplicar, operador }) {
  ...
  const cobrado = poliza.cuotas
    .filter((c) => c.estatus === "PAGADO")
    .reduce((s, c) => s + c.monto, 0);
  const adeudoMonto = poliza.cuotas
    .filter((c) => c.estatus === "ADEUDO")
    .reduce((s, c) => s + c.monto, 0);
  const porCobrar = poliza.cuotas
    .filter((c) => c.estatus === "PENDIENTE")
    .reduce((s, c) => s + c.monto, 0);

  const primerPendienteId = poliza.cuotas.find(
    (c) => c.estatus === "PENDIENTE",
  )?.id;
        ...
        >
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 sticky top-0 bg-white z-10">
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">
                Póliza <span className="font-mono font-bold">{poliza.id}</span>{" "}
                · {poliza.asegurado}
              </p>
            </div>
            <button
          ...
          <div className="p-6 space-y-5">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                {
                  label: "Forma de pago",
                  value: poliza.formaPago,
                  mono: false,
                },
                {
                  label: "Cobrado",
                  value: `$${cobrado.toFixed(2)}`,
                  mono: true,
                },
                {
                  label: "En adeudo",
                  value: `$${adeudoMonto.toFixed(2)}`,
                  mono: true,
                },
                {
                  label: "Por cobrar",
                  value: `$${porCobrar.toFixed(2)}`,
                  mono: true,
                },
              ].map((f) => (
                <div
                ...
                >
                  ...
                  <p
                    className={`text-sm font-bold ${f.mono ? "font-mono text-[#13193a]" : "text-[#13193a]"}`}
                  >
                    {f.value}
                  </p>
                </div>
            ...
            <div>
              <div className="flex justify-between text-xs text-gray-500 mb-1.5">
                ...
                <span>
                  {Math.round(((pagadas + enAdeudo) / totalCuotas) * 100)}%
                </span>
              </div>
              <div className="h-2 bg-gray-100 rounded-full overflow-hidden flex">
                <div
                  className="h-full bg-emerald-500 transition-all"
                  style={{ width: `${(pagadas / totalCuotas) * 100}%` }}
                />
                <div
                  className="h-full bg-blue-400 transition-all"
                  style={{ width: `${(enAdeudo / totalCuotas) * 100}%` }}
                />
              </div>
            </div>
            ...
            {!polizaBloq && bloqueada && (
              <div className="flex items-center gap-2 px-4 py-3 rounded-xl bg-gray-100 border border-gray-200 text-gray-500 text-xs font-semibold">
                <Lock className="w-4 h-4 shrink-0" />
                Póliza con {diasDesdeEmision} días desde la primera emisión —
                cobro bloqueado (máx. 45 días)
              </div>
            )}
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <p
                          className={`text-sm font-bold ${filaBloqueada ? "text-gray-400" : "text-[#13193a]"}`}
                        >
                          ${c.monto.toFixed(2)}
                        </p>
                        <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                          <p className="text-xs text-gray-400">
                            Vto. {c.vigencia ?? c.vto}
                          </p>
                          {(c.estatus === "PAGADO" || c.estatus === "ADEUDO") &&
                            c.fechaPago && (
                              <>
                                <span className="text-gray-300">·</span>
                                <p className="text-xs text-blue-600">
                                  Recibido {c.fechaPago}
                                </p>
                              </>
                            )}
                        </div>
...
export default function OperadorPagos({ usuario }) {
    ...
    try {
      ...
      const rowsBuilt = (polizasDB ?? []).map((pol) => {
        ...
        const nombreVendedor = pol.vendedores
          ? [pol.vendedores.nombre, pol.vendedores.apellido]
              .filter(Boolean)
              .join(" ")
          : "GAMAN";
      ...
      const oficinasSet = [
        ...new Set(
          (polizasDB ?? []).map((p) => p.oficinas?.nombre).filter(Boolean),
        ),
      ];
      const vendedoresSet = [
        ...new Set(rowsBuilt.map((r) => r.vendedor).filter(Boolean)),
      ];
  ...
  const abrirCuotas = useCallback(async (pol) => {
    ...
    setRows((prev) =>
      prev.map((r) => (r.polizaId === pol.polizaId ? { ...r, cuotas } : r)),
    );
    setPolizaSel((prev) =>
      prev?.polizaId === pol.polizaId ? { ...prev, cuotas } : prev,
    );
  }, []);
  ...
  const aplicarPago = async ({ cuotaId, polizaId, fecha, monto }) => {
    ...
    if (polizaSel) {
      ...
      setRows((prev) =>
        prev.map((r) =>
          r.polizaId === polizaSel.polizaId ? { ...r, cuotas } : r,
        ),
      );
  ...
  const filtradas = useMemo(() => {
    ...
    return rows.filter((p) => {
      const matchBusq =
        p.id.toLowerCase().includes(b) || p.asegurado.toLowerCase().includes(b);
      const matchOfic =
        filtroOficina === "Todas" || p.oficina === filtroOficina;
      const matchVend =
        filtroVendedor === "Todos" || p.vendedor === filtroVendedor;
      const matchFp =
        filtroFormaPago === "Todas" || p.formaPago === filtroFormaPago;
      const pendientes = p.cuotas
        ? p.cuotas.filter((c) => c.estatus === "PENDIENTE").length
        : -1;
      const matchEst =
        filtroEstado === "Todos" ||
        (filtroEstado === "Con pendientes" &&
          (pendientes === -1 || pendientes > 0)) ||
  ...
  const cuotasCobradas = cargadas.flatMap((p) =>
    p.cuotas.filter((c) => c.estatus === "PAGADO"),
  ).length;
  const totalCobrado = cargadas
    .flatMap((p) => p.cuotas.filter((c) => c.estatus === "PAGADO"))
    .reduce((s, c) => s + c.monto, 0);
  const cuotasAdeudo = cargadas.flatMap((p) =>
    p.cuotas.filter((c) => c.estatus === "ADEUDO"),
  ).length;
  const totalAdeudo = cargadas
    .flatMap((p) => p.cuotas.filter((c) => c.estatus === "ADEUDO"))
    .reduce((s, c) => s + c.monto, 0);
  const cuotasPend = cargadas.flatMap((p) =>
    p.cuotas.filter((c) => c.estatus === "PENDIENTE"),
  ).length;
  const montoPendiente = cargadas
    .flatMap((p) => p.cuotas.filter((c) => c.estatus === "PENDIENTE"))
    .reduce((s, c) => s + c.monto, 0);
  const vencidasList = cargadas.flatMap((p) =>
    p.cuotas.filter(
      (c) =>
        c.estatus === "PENDIENTE" &&
    ...
    <div className="p-6 py-3 min-h-full bg-gray-50 space-y-3">
      <div>
        ...
        <p className="text-gray-400 text-sm mt-0.5">
          Recepción y registro de pagos de cuotas
        </p>
      </div>
              ...
              {loading ? (
                ...
                pagosPag.map((p, i) => {
                  ...
                  return (
                    <tr
                      key={i}
                      className={`transition-colors ${polizaBloq ? "opacity-60 bg-gray-50/80" : "hover:bg-gray-50/60"}`}
                    >
                      <td className="px-5 py-1 font-mono text-xs font-bold text-[#13193a]">
                        {p.id}
                      </td>
                      <td className="px-5 py-1 text-xs font-semibold text-gray-700 whitespace-nowrap">
                        {p.asegurado}
                      </td>
                      <td className="px-5 py-1 text-xs text-gray-500 max-w-32 truncate">
                        {p.oficina}
                      </td>
                      <td className="px-5 py-1 text-xs text-gray-500 max-w-36 truncate">
                        {p.cobertura}
                      </td>
                      ...
                      <td className="px-5 py-1">
                        {!cargadas ? (
                          ...
                          <span
                            className={`text-xs font-bold ${hayVencida ? "text-red-600" : "text-amber-700"}`}
                          >
                            ${porCobrar.toFixed(2)}
                          </span>
                        ) : (
